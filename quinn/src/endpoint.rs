...
impl EndpointInner {
    fn drive_recv<'a>(&'a mut self, cx: &mut Context, now: Instant) -> Result<bool, io::Error> {
        ...
        loop {
            ...
            let socket_state = &mut self.socket_state;
            let socket = &self.socket;
            match socket.try_io(Interest::READABLE, || {
                socket_state.recv(socket.into(), &mut iovs, &mut metas)
            }) {
                Ok(msgs) => {
    ...
    fn drive_send(&mut self, cx: &mut Context) -> Result<bool, io::Error> {
        ...
        let result = loop {
            ...
            let socket_state = &mut self.socket_state;
            let socket = &self.socket;
            let udp_state = &self.udp_state;
            let transmits = self.outgoing.as_slices().0;
            match socket.try_io(Interest::WRITABLE, || {
                socket_state.send(socket.into(), udp_state, transmits)
            }) {
                Ok(n) => {
